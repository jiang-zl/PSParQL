...
public:
  ...
  bool locTable2queue() // 生成任务成功，则返回 true；否则，返回 false
  {
    ...
    if (global_vertex_pos < size) // 还能继续从本地顶点列表中生成任务
    {
      ...
      begin = -1; // meaning that local-table is exhausted 本地顶点列表中不能再生成新的任务
    global_vertex_pos_lock.unlock();
    //======== spawn tasks from local-table[begin, end)
    // 从本地顶点列表的 [begin, end) 区间内取出顶点生成任务
    if (begin == -1)
      return false; // 本地顶点列表中不能再生成新的任务，则返回 false
    else
    {
  ...
  bool pop_task()
  {
        ...
        { // CASE 2: try to move TASK_BATCH_NUM tasks from task-map to the queue
          ...
          { // i starts from 1 since push_task_from_taskmap() has been called once
            if (!push_task_from_taskmap())
              break; // task-map's "task_buf" is empty, no more try 就绪队列为空，则不再从中取任务
          }
        }
      } // 否则，磁盘文件不为空，已经通过 if(!file2queue()) 中的 file2queue() 将磁盘文件中的任务添加到任务队列中
    }
    //==================================
    // cout<<"q_task.size():"<<q_task.size()<<endl;
    // cout<<"q_delay_task.size():"<<q_delay_task.size()<<endl;
    if (q_delay_task.size() == 0 && q_task.size() == 0)
    {
      ...
      else
      {
        ...
        // cout<<"no task"<<endl;
        return false;
      } // 磁盘文件为空、本地顶点列表中没有顶点需要继续处理（不能生成新任务），且任务队列为空，则返回 false（实际就是没有任务可以继续执行）
    }
    ...
    // cout<<"generated_task_map.contains(key) = "<<key.first<<" "<<key.second<<endl;
  ...
  void run()
  {
    ...
    while (global_end_label == false) // otherwise, thread terminates
    {
        ...
        if (global_cache_size < VCACHE_LIMIT + VCACHE_OVERSIZE_LIMIT) //(1 + alpha) * vcache_limit
        {
          ...
          if (map_task.size < TASKMAP_LIMIT)
          {
            try
            {
              ...
              if (!pop_task())
                nothing_processed_by_pop = true; // only the last iteration is useful, others will be set back to false 任务队列 q_task 为空，不能生成新任务，没有可以继续处理的任务
            }
            catch (const std::exception &e)
            {
              cout << "pop_task 出错了" << endl;
              std::cerr << e.what() << '\n';
            }
          }
          else
            blocked = true; // only the last iteration is useful, others will be set back to false
        }
        else
          blocked = true; // only the last iteration is useful, others will be set back to false 只有在最后一轮迭代中有效
      }
      //------
      for (int i = 0; i < TASK_RECV_NUM; i++)
      {
        ...
        {
          if (!push_task_from_taskmap())
            nothing_to_push = true; // only the last iteration is useful, others will be set back to false 就绪任务队列为空
        }
        catch (const std::exception &e)
        {
          cout << "push_task_from_taskmap 出错了" << endl;
          std::cerr << e.what() << '\n';
        }
      }
      //------
      if (nothing_to_push)
      {
        // 就绪任务队列为空
        if (blocked)
          usleep(WAIT_TIME_WHEN_IDLE); // avoid busy-wait when idle 就绪任务队列为空，同时缓存表或任务 map 容量达到上限，则此时当前线程需要阻塞一段时间
        else if (nothing_processed_by_pop)
        {
          // cout<<"map_task.size"<<map_task.size<<endl;
