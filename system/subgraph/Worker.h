...
public:
  ...
  Worker(int comper_num, string local_disk_path = "buffered_tasks", string report_path = "report")
  {
    ...
    for (int i = 0; i < _num_workers; i++)
      req_counter[i] = 0; // must be before the next line
    // cout<<"_num_workers:"<<_num_workers<<endl;
  ...
  void load_graph(const char *inpath, VertexVec &vVec)
  {
    ...
    if (global_trimmer != NULL)
      trimmer = (TrimmerT *)global_trimmer;
    //------
    hdfsFS fs = getHdfsFS();
    hdfsFile in = getRHandle(inpath, fs);
    LineReader reader(fs, in);
    hash_map<VertexID, VertexT *> recorder;
    while (true)
    {
      reader.readLine();
      if (!reader.eof())
  ...
  void updateInEdgeVertex(hash_map<VertexID, VertexT *> &recorder,
                          VertexID inId, Label inVL, EdgeLabel inEL, const VertexT *outV)
  {
    VertexT *inV = nullptr;
    if (recorder.find(inId) == recorder.end())
    {
      inV = new VertexT;
      inV->id = inId;
      inV->value.l = inVL;
    ...
    vector<AdjItem> &nbs = inV->value.adj;
    AdjItem temp;
    temp.id = outV->id;
    temp.l = outV->value.l;
    temp.el = inEL;
  ...
  void updateVertex(char *line, hash_map<VertexID, VertexT *> &recorder)
  {
    ...
    VertexT *v = nullptr;
    if (recorder.find(id) == recorder.end())
    {
      v = new VertexT;
      v->id = id;
      v->value.l = vl;
  ...
  void sync_graph(VertexVec &vVec)
  {
    ...
    // // cout<< "保存分区结果:" <<endl;
    // int p_id = 0;
    // // cout << "_loaded_parts.size:" <<_loaded_parts.size()<<endl;
  ...
  void status_sync(bool sth2steal)
  {
    ...
    else
    {
      ...
      if (all_idle)
        global_end_label = true; // 只有当所有 worker 的工作状态为 true 时，all_idle 才为 true，即此时所有 worker 都已经工作结束
      for (int i = 0; i < _num_workers; i++)
      {
  ...
  */
  ...
  bool locTable2vec(vector<TaskT> &tvec)
  {
    ...
    while (tvec.size() < TASK_BATCH_NUM)
    {
      ...
      if (global_vertex_pos < size)
      {
        ...
        begin = -1; // meaning that local-table is exhausted 本地顶点已经用完
      global_vertex_pos_lock.unlock();
      //======== spawn tasks from local-table[begin, end)
      if (begin == -1)
        return false; // 本地顶点已经用完，返回 false
      else
      {
  ...
  bool file2vec(vector<TaskT> &tvec)
  {
    ...
    if (!succ)
      return false; //"global_file_list" is empty
    else
    {
      global_file_num--;
  ...
  bool steal_planning() // whether there's something to steal from/to others
  {
    ...
    else
    {
      ...
      {
        if (i != MASTER_RANK)
          remain_vec[i] = recv_data<size_t>(i, STATUS_CHANNEL); // 各个 slave worker 剩余的任务数量
        else
      ...
      while (!max_heap.empty() && !min_heap.empty()) // 只有当最大堆和最小堆中都有 worker 时才需要通过工作窃取来实现负载均衡（如果有一个为空，则说明所有的 worker 负载都较重或者较轻）
      {
        ...
        min_heap.pop();
        if (max.num_remain - TASK_BATCH_NUM < min.num_remain)
          break; // worker 之间负载相差较小，负载相对均衡，无需进行工作窃取。当 max.num_remain - min.num_remain < TASK_BATCH_NUM 时，负载最重的 worker 与负载最轻的 worker 两者之间的负载相差并不大，此时各个 worker 之间的负载相差较小，没有必要进行工作窃取
        else
        {                                   // worker 之间的负载相差较大，此时需要通过工作窃取实现负载均衡。每次工作窃取，是直接窃取数量为 TASK_BATCH_NUM 的一批任务
          ...
          //---
          if (max.num_remain > MIN_TASK_NUM_BEFORE_STEALING)
            max_heap.push(max); // 刚被窃取任务的 max 号 worker 剩余的任务数量依然大于阈值，则依然还需要其它 worker 窃取其任务，因此该 worker 依然加入到最大堆中
      ...
      {
        if (i == _my_rank)
          steal_lists[i].swap(my_steal_list); // 如果是当前 worker（master） ，则 swap ，从而将当前 worker 的任务窃取列表保存下来
        else
        {
    ...
    if (my_steal_list.size() == 0)
      return false; // 不需要窃取任务
    ...
    for (int i = 0; i < my_steal_list.size(); i++)
    {
      ...
      else
      {
        ...
        if (get_remaining_task_num() > MIN_TASK_NUM_BEFORE_STEALING)
          ...
          if (locTable2vec(tvec) == false)
            file2vec(tvec);                     // 先从本地顶点中获取任务，如果从本地无法生成任务，则从文件中生成任务
  ...
  void run(const WorkerParams &params)
  {
    ...
    // cout << "WORKER_TIMER = " << WORKER_TIMER << endl;

    // 初始化顶点查询顺序（在导入图数据之前导入查询图数据）
    // 导入查询图数据
    // 多机/多进程 -n 2/3 都能正常运行，多机MPI执行程序是正常的
    ...
    //   {
    //     if (EdgeDirect::IN != temp.d)
    //       continue;
    //     cout << "(id=" << temp.id << ", l=" << temp.l << ", el=" << temp.el << ") ";
    //   }
    //   cout << "}, out_adj={";
    //   for (const auto &temp : adj)
    //   {
    //     if (EdgeDirect::OUT != temp.d)
    //       continue;
    //     cout << "(id=" << temp.id << ", l=" << temp.l << ", el=" << temp.el << ") ";
    //   }
    //   cout << "}" << endl;
    ...
    if (global_aggregator != NULL)
      agg_thread = new AggSync<AggregatorT>;
    ...
    while (global_end_label == false)
    {
      ...
      for (int i = 0; i < num_compers; i++)
        idle_set[i] = false; // 在 Comper 中设置为 true，在 worker 中设置为 false
      global_num_idle = 0;
    ...
    cout << "Worker " << _my_rank << " finished" << endl;
    // 释放分组内存
    ...
    if (global_aggregator != NULL)
      delete agg_thread; // make sure destructor of agg_thread is called to do agg_sync() before exiting run()
    delete profiler;
  }
};
