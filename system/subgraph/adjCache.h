...
public:
                        ...
                        TaskMapT &taskmap, TaskT *task) // used when vcache miss happens, for adding the task to task_map 会调用 add2map ，即会将任务放进挂起任务 map 中
  {
    ...
    {                        // 远程顶点
      ...
      if (new_req)
        q_req.add(key); // 如果是一个新请求，则将该请求加入到请求队列中
      ret = NULL;
    }
    else
    { // 在本地缓存中找到远程顶点
      ...
      if (vpair.counter == 0)
        bucket.zeros.erase(key); // zero-cache.remove 当前顶点之前在 zeros 中（即之前没有被任务使用过），则需要将其从 zeros 中删除（因为该顶点会在本次任务中使用）
  ...
  void unlock(KeyType &key)
  {
    ...
    if (it->second.counter == 0)
      bucket.zeros.insert(key); // zero-cache.insert 因为已经没有任务使用 key 顶点，因此将该顶点放入 zeros 缓存中
    bucket.unlock();
  }
  ...
  size_t shrink(size_t num_to_delete, thread_counter &counter) // return value = how many elements failed to trim
  {
    ...
    while (num_to_delete > 0) // still need to delete element(s)
    {
      ...
      pos++;
      if (pos >= CONMAP_BUCKET_NUM)
        pos -= CONMAP_BUCKET_NUM; // 已经遍历到最后一个 bucket （此时 pos = CONMAP_BUCKET_NUM，而不会大于 CONMAP_BUCKET_NUM），则将 pos 重置为 0
      if (pos == start_pos)
        break; // finish one round of checking all zero-sets, no more checking （已经将 Z-tables、Γ-tables 中所有的 bucket 完整地遍历一遍，不再进行下一轮遍历，结束删除操作）
