  ...
  */
  ...
  {                                                         // called by Comper, giving its thread_counter and thread_id
      ...
      else // remote 远程顶点（有两种情况：一种是，该远程顶点已经被拉取过，并放入到缓存表中；另外一种是，该远程顶点未被拉取过，需要进行远程拉取）
      {
        ...
        if (remote_detected) // no need to call add2map(.) again 不需要再次调用 add2map（即不需要再次将 task_id 任务挂起）
        {
          ...
          if (frontier_vertexes[i] != NULL)
            met_counter++; // 第 i 个顶点已经拉取到本地，则 met_counter 加 1
        }
        else
        {
          // task_id 任务未被挂起
    ...
    if (remote_detected)
    {
      ...
      return true; // all v-local, continue to run the task for another iteration 所有顶点数据已全部拉取本地，可以继续下一轮迭代
  }
  ...
  void unlock_all()
  {
    ...
    for (int i = 0; i < frontier_vertexes.size(); i++)
    {
      ...
      if (hash(v->id) != _my_rank)
        vcache.unlock(v->id); // 如果是远程顶点，则需要在缓存表中更新相应的状态，即：将使用该顶点的任务数量减 1，如果减到 0，是该顶点会从 Γ-tables 移到 Z-tables 中
    }
  }
