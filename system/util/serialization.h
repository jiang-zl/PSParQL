...
ibinstream &operator<<(ibinstream &m, EdgeDirect d)
{ // EdgeDirect:char, i.e., in/out
  m.raw_bytes(&d, sizeof(EdgeDirect));
  return m;
}
...
obinstream &operator>>(obinstream &m, EdgeDirect &d)
{
  d = *(EdgeDirect *)m.raw_bytes(sizeof(EdgeDirect));
  return m;
}
